...
def create_pandas_and_matplotlib_dataframe_agent(
    ...
    **kwargs: Dict[str, Any],
    ...
    tools = [PythonAstREPLTool(locals={"df": df, "plt": plt})]
    now = pd.Timestamp.now().strftime("%Y-%m-%d_%H:%M:%S")
    prompt = ZeroShotAgent.create_prompt(
        tools, prefix=prefix.format(now=now), suffix=suffix_to_use, input_variables=input_variables
    )
